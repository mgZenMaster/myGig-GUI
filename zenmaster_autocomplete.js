  function get_text_id(optionslist_id, inputfield_id, id_field_id) {
  ...
  function update_optionslist(input) {
  ...
  function decorate_autocomplete(input) {
    ...
    input.oninput = function(event) {
      ...
      get_text_id(input.list.id, input.id, input.ajax_id_input.id);
    }
    input.onkeydown = function(event) {
        if(event.which == 13) event.preventDefault();
      };
    input.onkeyup = function(event) {
        update_optionslist(event.currentTarget);
      };
  }